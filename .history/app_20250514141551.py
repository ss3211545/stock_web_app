...
class TailMarketStockApp:
    ...
    def run_filter(self):
        ...
        # 在单独的线程中执行筛选，防止UI卡死
        self.filter_thread = threading.Thread(target=self._execute_filtering)
        self.filter_thread.daemon = True
        self.filter_thread.start()
    
    def _execute_filtering(self):
        """在线程中执行筛选操作"""
        try:
            # 检查API健康状态
            ...
            if len(stock_list) > 0:
                ...
                try:
                    ...
                    if "--mode" in sys.argv and sys.argv[sys.argv.index("--mode") + 1] == "test":
                        # 获取测试股票数量
                        test_stock_count = 5  # 默认值
                        for i, arg in enumerate(sys.argv):
                            if arg == "--test-stocks" and i + 1 < len(sys.argv):
                                try:
                                    test_stock_count = int(sys.argv[i + 1])
                                except:
                                    pass
                        
                        if test_stock_count > 0 and test_stock_count < len(stock_list):
                            stock_list = random.sample(stock_list, test_stock_count)
                            self.root.after(0, lambda: self._add_log(f"测试模式: 随机抽取{test_stock_count}只股票进行分析", "warning"))
                except Exception as e:
                    self.root.after(0, lambda: self._add_log(f"测试模式设置出错: {str(e)}", "warning"))
                    
                # 进行筛选
                result = self.data_fetcher.apply_all_filters(
                    stock_list, 
                ...
                if isinstance(result, tuple) and len(result) == 4:
                    filtered_stocks, detailed_info, max_step, partial_match = result
                else:
                    # 如果返回结果不符合预期，给予合理的默认值
                    self.root.after(0, lambda: self._add_log(f"筛选结果格式异常，使用默认值", "warning"))
                    if isinstance(result, tuple) and len(result) > 0:
                        filtered_stocks = result[0]
                    else:
                        filtered_stocks = []
                    detailed_info = []
                    max_step = 0
    ...
    def _show_user_friendly_message(self):
        """显示友好的用户提示，告知筛选过程正在进行"""
        # 选择一个随机的友好提示
        tips = [
            "正在对股票进行尾盘八大步骤筛选，请稍候...",
            "系统正在努力为您寻找符合条件的优质股票...",
            "正在分析市场数据，这可能需要一点时间...",
            "AI正在为您筛选潜在的投资机会，请耐心等待...",
            "数据分析进行中，优质股票即将呈现...",
            "股票筛选需要一点时间，我们正在寻找符合八大步骤的标的...",
            "正在进行多重筛选，为您找出最符合条件的股票...",
            "系统正在分析大量数据，请稍候片刻...",
            "正在运行尾盘选股算法，精选优质标的中..."
        ]
        tip = random.choice(tips)
        self.root.after(0, lambda: self._add_log(tip, "info"))
    ...
    def _update_animation_frame(self):
        ...
        step_name = self.step_descriptions[current_step]['title'] if current_step < len(self.step_descriptions) else "准备中"
        
        self._update_status(f"正在筛选: {step_name}{dots}")
        
        # 设置下一帧的定时器（500毫秒）
        self.progress_animation_id = self.root.after(500, self._update_animation_frame)
    
    def _update_filter_results(self):
        """更新筛选结果"""
        # 清除之前的列表框内容
        self.filtered_stocks_var.set([])
        ...
        for stock_info in self.filtered_stocks:
            ...
            increase = stock_info.get('increase', 0)
            price = stock_info.get('price', 0)
            display = f"{code} {name} {price:.2f} {increase:.2f}%"
            stock_display.append(display)
        
        # 更新列表框
        self.filtered_stocks_var.set(stock_display)
        
        # 更新结果统计
        total_count = len(self.filtered_stocks)
        if total_count > 0:
            # 有符合条件的股票
            message = f"筛选完成，找到{total_count}只符合条件的股票"
            self._add_log(message, "success")
            self._update_status(message)
            
            # 显示筛选统计
            self._show_summary()
        elif self.partial_match and self.max_step > 0:
            # 部分条件匹配
            message = f"没有股票符合全部条件，但有{len(self.detailed_info)}只股票符合前{self.max_step}个步骤"
            self._add_log(message, "warning")
            self._update_status(message)
            
            # 显示部分匹配的结果
            self._show_partial_match_results()
        else:
            # 没有符合条件的股票
            message = "筛选完成，没有找到符合条件的股票"
            self._add_log(message, "info")
            self._update_status(message)
    
    def _show_summary(self):
        """显示筛选统计"""
        summary = f"筛选完成，找到{len(self.filtered_stocks)}只符合条件的股票\n\n"
        summary += "数据质量统计:\n"
        ...
        # 添加日志
        self._add_log("筛选完成，成功找到符合条件的股票", "success")
    ...
    def _update_detail_info(self, stock_info):
        ...
        # 成交量数据
        detail_text += f"\n交易数据:\n"
        detail_text += f"成交量: {format(stock_info.get('volume', 0), ',')} [可靠性: HIGH]\n"
    ...
    def _update_summary_info(self):
        ...
        # 获取市场
        market_str = self.selected_market.get()
        market_display = "A股(沪深市场)" if market_str == "A股" else market_str
        
        # 构建摘要
        summary = f"# 尾盘选股八大步骤分析结果\n\n"
        summary += f"分析时间: {today} {time_str}\n"
        summary += f"分析市场: {market_display}\n"
        
        # 股票数量统计
        if market_str == "A股":
            sh_count = len(self.data_fetcher.get_stock_list("SH"))
            sz_count = len(self.data_fetcher.get_stock_list("SZ"))
            total_count = sh_count + sz_count
            summary += f"初始股票数: {total_count} (上证: {sh_count}, 深证: {sz_count})\n"
        else:
            summary += f"初始股票数: {len(self.data_fetcher.get_stock_list(self.selected_market.get()))}\n"
        
        # 筛选结果统计
        if self.filtered_stocks:
            summary += f"筛选后剩余: {len(self.filtered_stocks)}只股票\n"
            
            # 计算筛选率
            if market_str == "A股":
                filter_rate = (1 - len(self.filtered_stocks) / total_count) * 100
            else:
                total = len(self.data_fetcher.get_stock_list(self.selected_market.get()))
                filter_rate = (1 - len(self.filtered_stocks) / total) * 100
                
            summary += f"筛选率: {filter_rate:.2f}%\n"
            
            # 部分匹配信息
            if self.partial_match:
                summary += f"\n注意: 未找到完全符合八大步骤的股票\n"
                summary += f"显示的是符合前{self.max_step}步筛选条件的股票\n"
        else:
            summary += "未找到符合条件的股票\n"
        ...
        # 添加各步骤的详细信息
        for i, step in enumerate(self.step_descriptions):
            if i < len(self.filter_steps_data) - 1:  # 检查是否有此步骤的数据
                data = self.filter_steps_data[i+1]  # +1是因为filter_steps_data第一项是初始数据
                status_text = "通过" if data['status'] == 'success' else "未通过"
                prev_count = self.filter_steps_data[i]['count']
                curr_count = data['count']
                filter_rate = ((prev_count - curr_count) / prev_count * 100) if prev_count > 0 else 0
                
                summary += f"{i+1}. {step['title']}: {status_text}, 剩余 {curr_count} 只, 此步筛除率 {filter_rate:.2f}%\n"
                summary += f"   条件: {step['condition']}\n"
                summary += f"   说明: {step['pro_explanation']}\n\n"
        ...
        if self.filtered_stocks:
            if self.partial_match:
                self._update_status(f"筛选完成，符合前{self.max_step}步的股票 {len(self.filtered_stocks)} 只")
            else:
                self._update_status(f"筛选完成，符合八大步骤的股票 {len(self.filtered_stocks)} 只")
        else:
            self._update_status("未找到符合条件的股票")

    def _reset_filter_visualization(self):
        """重置筛选过程可视化"""
        # 查找筛选过程可视化面板
        filter_vis_frame = None
                    ...
                    if isinstance(child, ttk.Frame):
                        ...
                        for grandchild in child.winfo_children():
                            if isinstance(grandchild, ttk.Notebook):
                                # 查找筛选过程可视化标签页
                                for i in range(grandchild.index("end")):
                                    tab_name = grandchild.tab(i, "text")
                                    if tab_name == "筛选过程可视化":
                                        filter_vis_frame = grandchild.winfo_children()[i]
                                        break
        
        if not filter_vis_frame:
            # 如果找不到筛选过程可视化面板，寻找其他可能的容器
            for widget in self.root.winfo_children():
                if isinstance(widget, ttk.PanedWindow):
                    for pane in widget.panes():
                        pane_widget = widget.nametowidget(pane)
                        if isinstance(pane_widget, ttk.Frame):
                            for child in pane_widget.winfo_children():
                                if isinstance(child, ttk.Notebook):
                                    # 尝试使用不同的方法查找标签页
                                    for tab in child.tabs():
                                        if child.tab(tab, "text") == "筛选过程可视化":
                                            filter_vis_frame = child.nametowidget(tab)
                                            break
                                # 直接查找可能的筛选过程可视化框架
                                if child.winfo_name().endswith("filter_vis_frame"):
                                    filter_vis_frame = child
                                    break
        
        # 如果仍然找不到，创建一个新的占位框架
        if not filter_vis_frame:
            print("筛选过程可视化面板不存在，稍后将创建")
            # 仅删除过滤步骤数据
            self.filter_steps_data = []
            self.current_step = 0
            return
        ...
        # 清空筛选步骤容器内的所有组件
        for widget in filter_vis_frame.winfo_children():
            widget.destroy()
        ...
        self.filter_progress.pack(pady=10, padx=10, fill=tk.X)
        ...
        # 创建步骤容器
        steps_container = ttk.Frame(filter_vis_frame, style="TFrame")
        steps_container.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        # 创建八大步骤显示
        for i, step in enumerate(self.step_descriptions):
            step_frame = ttk.Frame(steps_container, style="TFrame")
            step_frame.pack(fill=tk.X, pady=5)
            
            # 步骤标题行
            header_frame = ttk.Frame(step_frame, style="TFrame")
            ...
            title_label = ttk.Label(
                header_frame, 
                text=f"{i+1}. {step['title']} ({step['condition']})",
                font=("Arial", 12, "bold"),
                background=THEME_COLOR
            )
            ...
            stock_count_label.pack(side=tk.RIGHT, padx=10)
            
            # 存储状态标签引用
            step["waiting_label"] = waiting_label
            step["in_progress_label"] = in_progress_label
            step["success_label"] = success_label
            step["fail_label"] = fail_label
            step["stock_count_label"] = stock_count_label
            
            # 步骤详情行
            detail_frame = ttk.Frame(step_frame, style="TFrame")
            detail_frame.pack(fill=tk.X, padx=30)
            
            # 专业解释
            pro_label = ttk.Label(
                detail_frame,
                text=step["pro_explanation"],
                wraplength=550,
                justify=tk.LEFT,
                background=THEME_COLOR
            )
            pro_label.pack(anchor=tk.W, pady=(5, 0))
            
            # 简单解释
            simple_label = ttk.Label(
                detail_frame,
                text=f"通俗解释: {step['simple_explanation']}",
                wraplength=550,
                justify=tk.LEFT,
                foreground="#666666",
                font=("Arial", 9, "italic"),
            ...
            simple_label.pack(anchor=tk.W, pady=(2, 5))
            
            # 分隔线
            if i < len(self.step_descriptions) - 1:
                separator = ttk.Separator(steps_container, orient=tk.HORIZONTAL)
    ...
    def _update_filter_step(self, step_index, status, stock_count, total_stocks=None):
        ...
        try:
            # 隐藏所有状态标签
            if hasattr(step, 'waiting_label') and step.get('waiting_label') and step['waiting_label'].winfo_exists():
            ...
            # 更新股票数量标签
            if hasattr(step, 'stock_count_label') and step.get('stock_count_label') and step['stock_count_label'].winfo_exists():
            ...
            # 出现异常，但我们不希望中断筛选过程
            print(f"更新筛选步骤UI出错: {e}")
        
        # 更新进度条（如果存在）
        try:
            if hasattr(self, 'filter_progress') and self.filter_progress.winfo_exists():
                if status == 'in_progress':
    ...
    def _get_stock_detail_text(self, stock_info):
        """获取股票详细信息文本"""
        if not stock_info:
            return ""
        ...
        # 数据时间
        if 'data_time' in stock_info:
            detail_text += f"数据时间: {stock_info['data_time']}\n"
        
        # 数据质量
        if 'data_status' in stock_info and 'reliability' in stock_info:
            detail_text += f"\n数据质量:\n"
            detail_text += f"完整性: {stock_info['data_status']}\n"
            detail_text += f"可靠性: {stock_info['reliability']}\n"
        
        # 基本指标
        detail_text += f"\n基本指标:\n"
        detail_text += f"量比: {stock_info.get('volume_ratio', 'N/A')}\n"
        detail_text += f"换手率: {stock_info.get('turnover_rate', 'N/A')}%\n"
        detail_text += f"市值: {stock_info.get('market_cap', 'N/A')}亿\n"
        ...
        # 财务指标
        if 'financial' in stock_info:
            detail_text += f"\n财务指标:\n"
            for key, value in stock_info['financial'].items():
                detail_text += f"{key}: {value}\n"
        
        # 筛选步骤
        if 'step_data' in stock_info:
            detail_text += f"\n筛选步骤结果:\n"
            for step, result in stock_info['step_data'].items():
                if result:
                    icon = "✅"
                else:
                    icon = "❌"
                detail_text += f"{icon} 步骤{step}: {result}\n"
        
        return detail_text

    def _show_partial_match_results(self):
        """显示部分匹配的结果"""
        summary = f"⚠️ 警告：未找到完全符合八大步骤的股票\n\n"
        summary += f"显示的是符合前{self.max_step}步条件的股票\n"
        summary += f"共{len(self.detailed_info)}只股票\n\n"
        summary += f"数据质量统计:\n"
        summary += self._get_data_quality_summary()
        summary += f"\n完成时间: {datetime.now().strftime('%H:%M:%S')}"
    ...
    def _get_data_quality_summary(self):
        ...
        for stock in self.detailed_info:
            if stock.get('reliability') == 'HIGH' and stock.get('data_status') == 'COMPLETE':
                high_quality += 1
            elif stock.get('reliability') == 'MEDIUM' or stock.get('data_status') == 'PARTIAL':
                medium_quality += 1
            else:
                low_quality += 1
        
        summary = f"完全可靠: {high_quality}只\n"
        summary += f"部分可靠: {medium_quality}只\n"
        summary += f"低可靠性: {low_quality}只\n"
...
def test_mode():
    """检查是否处于测试模式"""
    import sys
    return "--mode" in sys.argv and sys.argv[sys.argv.index("--mode") + 1] == "test"
