...
class TailMarketStockApp:
    ...
    def _update_degradation_settings(self):
        ...
        self._add_log(f"数据降级策略已更新: {'启用' if enabled else '禁用'}, 级别: {level}", "info")
    ...
    def run_filter(self):
        ...
        try:
            ...
            # 初始化结果列表，保存每一步的筛选结果
            steps_results = []
            current_steps_results = []
            all_steps_passed = True
            
            # 步骤1: 涨幅在3%到5%之间
            self._add_log("步骤1: 筛选涨幅3%到5%的股票", "info")
            self._add_log(f"筛选前股票数量: {len(stock_list)}", "info")
            filtered_stocks = self.data_fetcher.filter_by_price_increase(stock_list)
            self._add_log(f"筛选后股票数量: {len(filtered_stocks)}", "info")
            if filtered_stocks:
                self._add_log(f"通过筛选的股票: {', '.join(filtered_stocks[:5])}" + ("..." if len(filtered_stocks) > 5 else ""), "info")
            steps_results.append(filtered_stocks)
            current_steps_results = filtered_stocks
            if not filtered_stocks:
                all_steps_passed = False
                self._add_log("步骤1没有符合条件的股票，但将继续执行后续步骤", "warning")
                # 使用前一步的结果继续（这里是原始股票列表）
                current_steps_results = stock_list[:20]  # 取前20个继续
            
            # 步骤2: 量比大于1
            self._add_log("步骤2: 筛选量比大于1的股票", "info")
            self._add_log(f"筛选前股票数量: {len(current_steps_results)}", "info")
            filtered_stocks = self.data_fetcher.filter_by_volume_ratio(current_steps_results)
            self._add_log(f"筛选后股票数量: {len(filtered_stocks)}", "info")
            if filtered_stocks:
                self._add_log(f"通过筛选的股票: {', '.join(filtered_stocks[:5])}" + ("..." if len(filtered_stocks) > 5 else ""), "info")
            steps_results.append(filtered_stocks)
            if not filtered_stocks:
                all_steps_passed = False
                self._add_log("步骤2没有符合条件的股票，但将继续执行后续步骤", "warning")
                # 使用前一步的结果继续
                filtered_stocks = current_steps_results[:20]  # 取前20个继续
            current_steps_results = filtered_stocks
            
            # 步骤3: 换手率在5%到10%之间
            self._add_log("步骤3: 筛选换手率在5%到10%之间的股票", "info")
            self._add_log(f"筛选前股票数量: {len(current_steps_results)}", "info")
            filtered_stocks = self.data_fetcher.filter_by_turnover_rate(current_steps_results)
            self._add_log(f"筛选后股票数量: {len(filtered_stocks)}", "info")
            if filtered_stocks:
                self._add_log(f"通过筛选的股票: {', '.join(filtered_stocks[:5])}" + ("..." if len(filtered_stocks) > 5 else ""), "info")
            steps_results.append(filtered_stocks)
            if not filtered_stocks:
                all_steps_passed = False
                self._add_log("步骤3没有符合条件的股票，但将继续执行后续步骤", "warning")
                # 使用前一步的结果继续
                filtered_stocks = current_steps_results[:20]  # 取前20个继续
            current_steps_results = filtered_stocks
            
            # 步骤4: 市值在50亿到200亿之间
            self._add_log("步骤4: 筛选市值在50亿到200亿之间的股票", "info")
            self._add_log(f"筛选前股票数量: {len(current_steps_results)}", "info")
            filtered_stocks = self.data_fetcher.filter_by_market_cap(current_steps_results)
            self._add_log(f"筛选后股票数量: {len(filtered_stocks)}", "info")
            if filtered_stocks:
                self._add_log(f"通过筛选的股票: {', '.join(filtered_stocks[:5])}" + ("..." if len(filtered_stocks) > 5 else ""), "info")
            steps_results.append(filtered_stocks)
            if not filtered_stocks:
                all_steps_passed = False
                self._add_log("步骤4没有符合条件的股票，但将继续执行后续步骤", "warning")
                # 使用前一步的结果继续
                filtered_stocks = current_steps_results[:20]  # 取前20个继续
            current_steps_results = filtered_stocks
            ...
            self._add_log(f"筛选前股票数量: {len(current_steps_results)}", "info")
            filtered_stocks = self.data_fetcher.filter_by_increasing_volume(current_steps_results)
            self._add_log(f"筛选后股票数量: {len(filtered_stocks)}", "info")
            if filtered_stocks:
                self._add_log(f"通过筛选的股票: {', '.join(filtered_stocks[:5])}" + ("..." if len(filtered_stocks) > 5 else ""), "info")
            steps_results.append(filtered_stocks)
            if not filtered_stocks:
                all_steps_passed = False
                self._add_log("步骤5没有符合条件的股票，但将继续执行后续步骤", "warning")
                # 使用前一步的结果继续
                filtered_stocks = current_steps_results[:20]  # 取前20个继续
            current_steps_results = filtered_stocks
            ...
            self._add_log(f"筛选前股票数量: {len(current_steps_results)}", "info")
            filtered_stocks = self.data_fetcher.filter_by_moving_averages(current_steps_results)
            self._add_log(f"筛选后股票数量: {len(filtered_stocks)}", "info")
            if filtered_stocks:
                self._add_log(f"通过筛选的股票: {', '.join(filtered_stocks[:5])}" + ("..." if len(filtered_stocks) > 5 else ""), "info")
            steps_results.append(filtered_stocks)
            if not filtered_stocks:
                all_steps_passed = False
                self._add_log("步骤6没有符合条件的股票，但将继续执行后续步骤", "warning")
                # 使用前一步的结果继续
                filtered_stocks = current_steps_results[:20]  # 取前20个继续
            current_steps_results = filtered_stocks
            
            # 步骤7: 强于大盘
            self._add_log("步骤7: 筛选表现强于大盘的股票", "info")
            self._add_log(f"筛选前股票数量: {len(current_steps_results)}", "info")
            filtered_stocks = self.data_fetcher.filter_by_market_strength(current_steps_results)
            self._add_log(f"筛选后股票数量: {len(filtered_stocks)}", "info")
            if filtered_stocks:
                self._add_log(f"通过筛选的股票: {', '.join(filtered_stocks[:5])}" + ("..." if len(filtered_stocks) > 5 else ""), "info")
            steps_results.append(filtered_stocks)
            if not filtered_stocks:
                all_steps_passed = False
                self._add_log("步骤7没有符合条件的股票，但将继续执行后续步骤", "warning")
                # 使用前一步的结果继续
                filtered_stocks = current_steps_results[:20]  # 取前20个继续
            current_steps_results = filtered_stocks
            ...
            self._add_log(f"筛选前股票数量: {len(current_steps_results)}", "info")
            filtered_stocks = self.data_fetcher.filter_by_tail_market_high(current_steps_results)
            self._add_log(f"筛选后股票数量: {len(filtered_stocks)}", "info")
            if filtered_stocks:
                self._add_log(f"通过筛选的股票: {', '.join(filtered_stocks[:5])}" + ("..." if len(filtered_stocks) > 5 else ""), "info")
            steps_results.append(filtered_stocks)
            if not filtered_stocks:
                all_steps_passed = False
                self._add_log("步骤8没有符合条件的股票", "warning")
                # 使用前一步的结果
                filtered_stocks = current_steps_results[:20]  # 取前20个
            
            # 保存筛选结果
            self.filtered_stocks = filtered_stocks
            
            # 标记是否部分匹配
            self.partial_match = not all_steps_passed
            max_step = 8
            for i in range(8):
                if not steps_results[i]:
                    max_step = i
                    break
            ...
            if all_steps_passed:
                ...
                self._add_log(f"筛选完成，但不是所有步骤都有符合条件的股票", "warning")
                self._add_log(f"显示最后一步的筛选结果，共 {len(filtered_stocks)} 只股票", "info")
    ...
    def _add_log(self, message, level="info"):
        ...
        message: str
            日志消息
        level: str
            日志级别，可选 'info', 'warning', 'error', 'success'
        """
        self.result_text.config(state=tk.NORMAL)
