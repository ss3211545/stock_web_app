                    ...
                    try:
                        ...
                        try:
                            ...
                            if realtime_data and len(realtime_data) > 0:
                                ...
                                if (stock_data.get('price', 0) > stock_data.get('pre_close', 0) and
                                    (stock_data.get('high', 0) - stock_data.get('low', 0)) / stock_data.get('pre_close', 0) > 0.02):
                                    filtered_stocks.append(code)
                                    logger.info(f"{code}当前价格高于昨收，日内波动较大，视为短期均线上扬")
                        except Exception as e:
                            logger.error(f"尝试使用实时数据判断{code}均线状态失败: {str(e)}")
                            continue
