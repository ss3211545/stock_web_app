...
class StockDataFetcher:
    ...
    def filter_by_increasing_volume(self, stock_codes):
        ...
        """
        if not stock_codes:
            logger.warning("输入的股票列表为空，无法进行成交量筛选")
            return []
        ...
        try:
            ...
            logger.info(f"成交量筛选: 分{total_batches}批处理{len(stock_codes)}只股票的成交量分析")
            ...
            for batch_idx in range(total_batches):
                ...
                logger.info(f"成交量筛选: 处理第{batch_idx+1}/{total_batches}批，{len(batch_codes)}只股票")
                logger.info(f"当前批次股票: {', '.join(batch_codes)}")
                
                for code in batch_codes:
                    try:
                        # 使用实时数据来处理，而不是K线数据 (绕过K线数据获取问题)
                        logger.info(f"成交量筛选: 使用实时数据分析{code}的成交量趋势")
                        ...
                        try:
                            ...
                            if realtime_data and len(realtime_data) > 0 and realtime_data[0].get('volume', 0) > 0:
                                ...
                                volume_ratio = extra_info.get('volume_ratio', 0)
                                logger.info(f"成交量筛选: {code}的量比为{volume_ratio}")
                                
                                # 如果量比大于1.5，视为成交量放大
                                if volume_ratio > 1.5:
                                    filtered_stocks.append(code)
                                    logger.info(f"成交量筛选: {code}量比大于1.5，视为成交量放大，通过筛选")
                                else:
                                    logger.info(f"成交量筛选: {code}量比为{volume_ratio}，小于1.5，未通过筛选")
                            else:
                                logger.warning(f"成交量筛选: {code}无法获取有效的成交量数据")
                        except Exception as e:
                            logger.error(f"成交量筛选: 尝试使用实时数据判断{code}成交量趋势失败: {str(e)}")
                            # 记录数据质量问题
                        ...
                        logger.error(f"成交量筛选: 分析{code}的成交量数据时出现错误: {str(e)}")
                        # 记录数据质量问题
                        stocks_data_quality[code] = {
                            'source': 'UNKNOWN',
                            'reliability': 'NONE',
            ...
            logger.info(f"成交量筛选: 从{len(stock_codes)}只股票中筛选出{len(filtered_stocks)}只符合条件的股票")
            if filtered_stocks:
                logger.info(f"成交量筛选: 通过筛选的股票: {', '.join(filtered_stocks)}")
            else:
                logger.warning(f"成交量筛选: 没有找到符合条件的股票")
            
            return filtered_stocks
            
        except Exception as e:
            logger.error(f"成交量筛选过程中出错: {str(e)}")
            logger.exception("成交量筛选详细错误堆栈:")
    ...
    def filter_by_moving_averages(self, stock_codes):
        ...
        """
        if not stock_codes:
            logger.warning("输入的股票列表为空，无法进行均线筛选")
            return []
        ...
        try:
            ...
            logger.info(f"均线筛选: 分{total_batches}批处理{len(stock_codes)}只股票的均线分析")
            ...
            for batch_idx in range(total_batches):
                ...
                logger.info(f"均线筛选: 处理第{batch_idx+1}/{total_batches}批，{len(batch_codes)}只股票")
                logger.info(f"均线筛选: 当前批次股票: {', '.join(batch_codes)}")
                
                for code in batch_codes:
                    try:
                        # 使用实时数据分析
                        logger.info(f"均线筛选: 使用实时数据分析{code}的均线状态")
                        ...
                        try:
                            ...
                            if realtime_data and len(realtime_data) > 0:
                                ...
                                logger.info(f"均线筛选: {code}当前价格={price}, 昨收={pre_close}, 今日高点={high}, 今日低点={low}")
                                
                                # 由于无法获取K线数据，使用价格位置判断
                                # 如果价格高于昨日收盘价并且今日高点/低点范围较大，视为短期均线上扬
                                price_higher = price > pre_close
                                volatility = (high - low) / pre_close if pre_close > 0 else 0
                                
                                logger.info(f"均线筛选: {code}价格高于昨收: {price_higher}, 波动率: {volatility:.2%}")
                                
                                if price_higher and volatility > 0.02:
                                    filtered_stocks.append(code)
                                    logger.info(f"均线筛选: {code}当前价格高于昨收，日内波动={volatility:.2%}较大，视为短期均线上扬，通过筛选")
                                else:
                                    logger.info(f"均线筛选: {code}未通过均线筛选, 价格高于昨收: {price_higher}, 波动率: {volatility:.2%}")
                            else:
                                logger.warning(f"均线筛选: {code}无法获取有效的价格数据")
                        except Exception as e:
                            logger.error(f"均线筛选: 尝试使用实时数据判断{code}均线状态失败: {str(e)}")
                            # 记录数据质量问题
                            stocks_data_quality[code] = {
                                'source': 'UNKNOWN',
                                'reliability': 'LOW',
                        ...
                        logger.error(f"均线筛选: 分析{code}的均线数据时出现错误: {str(e)}")
                        # 记录数据质量问题
                        stocks_data_quality[code] = {
                            'source': 'UNKNOWN',
                            'reliability': 'NONE',
            ...
            logger.info(f"均线筛选: 从{len(stock_codes)}只股票中筛选出{len(filtered_stocks)}只符合条件的股票")
            if filtered_stocks:
                logger.info(f"均线筛选: 通过筛选的股票: {', '.join(filtered_stocks)}")
            else:
                logger.warning(f"均线筛选: 没有找到符合条件的股票")
            
            return filtered_stocks
            
        except Exception as e:
            logger.error(f"均线筛选过程中出错: {str(e)}")
            logger.exception("均线筛选详细错误堆栈:")
            # 出错时明确标记数据问题，但不返回任何随机股票
            return []
    ...
    def filter_by_market_strength(self, stock_codes):
        ...
        """
        if not stock_codes:
            logger.warning("输入的股票列表为空，无法进行大盘强弱筛选")
            return []
        ...
        try:
            ...
            logger.info(f"大盘强弱筛选: 获取{market}市场指数{index_code}数据")
            
            index_data = self.get_realtime_data([index_code])
            
            if not index_data:
                logger.warning(f"大盘强弱筛选: 未能获取{market}市场指数数据，使用默认涨跌幅0.5%")
                # 如果获取不到指数数据，模拟一个涨跌幅
                index_change_pct = 0.5
            else:
                # 获取大盘涨跌幅
                index_change_pct = index_data[0]['change_pct']
                logger.info(f"大盘强弱筛选: {market}市场指数{index_code}当前涨跌幅为{index_change_pct:.2f}%")
            
            # 获取个股行情
            logger.info(f"大盘强弱筛选: 获取{len(stock_codes)}只股票的实时数据")
            stock_data = self.get_realtime_data(stock_codes)
            ...
            for stock in stock_data:
                ...
                logger.info(f"大盘强弱筛选: {stock_code}涨跌幅为{stock_change_pct:.2f}%, 大盘指数涨跌幅为{index_change_pct:.2f}%")
                
                if stock_change_pct > index_change_pct:
                    filtered_stocks.append(stock_code)
                    logger.info(f"大盘强弱筛选: {stock_code}涨幅{stock_change_pct:.2f}%大于大盘{index_change_pct:.2f}%，通过筛选")
                else:
                    logger.info(f"大盘强弱筛选: {stock_code}涨幅{stock_change_pct:.2f}%不高于大盘{index_change_pct:.2f}%，未通过筛选")
            
            logger.info(f"大盘强弱筛选: 从{len(stock_codes)}只股票中筛选出{len(filtered_stocks)}只表现强于大盘的股票")
            if filtered_stocks:
                logger.info(f"大盘强弱筛选: 通过筛选的股票: {', '.join(filtered_stocks)}")
            else:
                logger.warning(f"大盘强弱筛选: 没有找到涨幅强于大盘的股票")
            
            return filtered_stocks
            
        except Exception as e:
            logger.error(f"大盘强弱筛选过程中出错: {str(e)}")
            logger.exception("大盘强弱筛选详细错误堆栈:")
            return []
    ...
    def filter_by_tail_market_high(self, stock_codes):
        ...
        """
        if not stock_codes:
            logger.warning("输入的股票列表为空，无法进行尾盘创新高筛选")
            return []
            
        try:
            # 检查是否处于尾盘时间
            is_tail_market = self.is_tail_market_time()
            logger.info(f"尾盘创新高筛选: 当前是否处于尾盘时间: {is_tail_market}")
            
            # 如果不在尾盘时间，根据设置决定是否继续
            if not is_tail_market:
                logger.warning("尾盘创新高筛选: 当前不是尾盘时间，使用模拟数据")
            
            # 获取实时数据
            logger.info(f"尾盘创新高筛选: 获取{len(stock_codes)}只股票的实时数据")
            stock_data = self.get_realtime_data(stock_codes)
            ...
            for stock in stock_data:
                ...
                # 判断条件：当前价格接近今日最高价（相差不超过0.5%）
                price_diff_ratio = (high_price - current_price) / high_price if high_price > 0 else 1.0
                is_near_high = price_diff_ratio <= 0.005  # 不超过0.5%的差距
                
                logger.info(f"尾盘创新高筛选: {code}当前价格={current_price}, 今日最高价={high_price}, 差距比例={price_diff_ratio:.2%}")
                
                if is_near_high:
                    filtered_stocks.append(code)
                    logger.info(f"尾盘创新高筛选: {code}当前价格接近今日最高价, 差距比例={price_diff_ratio:.2%}, 视为尾盘创新高，通过筛选")
                else:
                    logger.info(f"尾盘创新高筛选: {code}当前价格与今日最高价差距较大, 差距比例={price_diff_ratio:.2%}, 未通过筛选")
            
            logger.info(f"尾盘创新高筛选: 从{len(stock_codes)}只股票中筛选出{len(filtered_stocks)}只尾盘创新高的股票")
            if filtered_stocks:
                logger.info(f"尾盘创新高筛选: 通过筛选的股票: {', '.join(filtered_stocks)}")
            else:
                logger.warning(f"尾盘创新高筛选: 没有找到尾盘创新高的股票")
            
            return filtered_stocks
            
        except Exception as e:
            logger.error(f"尾盘创新高筛选过程中出错: {str(e)}")
            logger.exception("尾盘创新高筛选详细错误堆栈:")
            return []
    ...
    def get_kline_data(self, stock_code, kline_type=1, num_periods=60):
        ...
        if cache_key in self.kline_cache and not self.is_update_needed('kline', cache_key):
            logger.info(f"K线数据: 使用缓存的{stock_code} K线数据")
            return self.kline_cache[cache_key]
            
        # 直接返回空数据而不调用其他方法
        logger.warning(f"K线数据: {stock_code}的K线数据获取功能暂不可用，详细原因如下")
        logger.warning(f"K线数据: 由于数据源稳定性问题，K线数据获取功能已经被禁用")
        logger.warning(f"K线数据: 系统将继续使用实时数据来替代K线数据，数据质量可能会下降")
        logger.warning(f"K线数据: 如需解决，请联系管理员修复数据源接口问题")
    ...
    def _get_kline_from_sina(self, stock_code, kline_type=1, num_periods=60):
        """
        从新浪财经获取K线数据
        ...
        try:
            ...
            logger.error(f"从新浪获取K线数据出错: {str(e)}")
            return []
    
    def _get_kline_from_eastmoney(self, stock_code, kline_type=1, num_periods=60):
        """
        从东方财富获取K线数据
        ...
        try:
                ...
                if 'data' in json_data and 'klines' in json_data['data']:
                    ...
                    for item_str in data:
                        try:
                            ...
                            logger.error(f"解析东方财富K线单条数据出错: {str(e)}")
                            continue
            ...
            logger.error(f"从东方财富获取K线数据出错: {str(e)}")
            return []

    def _get_kline_from_akshare(self, stock_code, kline_type=1, num_periods=60):
        """
        使用AKShare获取K线数据
        ...
        if not AKSHARE_AVAILABLE:
            logger.warning("AKShare不可用，无法获取K线数据")
            return []
        ...
        try:
            ...
            if kline_type == 1:  # 日K
                df = ak.stock_zh_a_hist(
                    ...
                    period="daily", 
                    adjust="qfq", 
                    start_date=(datetime.now() - timedelta(days=num_periods*2)).strftime('%Y%m%d'),
                ...
                df = ak.stock_zh_a_hist(
                    ...
                    period="weekly", 
                    adjust="qfq", 
                    start_date=(datetime.now() - timedelta(days=num_periods*14)).strftime('%Y%m%d'),
                    end_date=datetime.now().strftime('%Y%m%d')
                )
                ...
                df = ak.stock_zh_a_hist(
                    ...
                    period="monthly", 
                    adjust="qfq", 
                    start_date=(datetime.now() - timedelta(days=num_periods*30)).strftime('%Y%m%d'),
                    end_date=datetime.now().strftime('%Y%m%d')
                )
