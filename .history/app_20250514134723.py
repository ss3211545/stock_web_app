...
class TailMarketStockApp:
    ...
    def __init__(self, root):
        ...
        # 市场选择和当前选中股票
        self.selected_market = tk.StringVar(value="A股")
    ...
    def _init_ui(self):
        ...
        markets = [("A股(上证+深证)", "A股"), ("上证", "SH"), ("深证", "SZ"), ("北证", "BJ"), ("港股", "HK"), ("美股", "US")]
    ...
    def _execute_filter_process(self):
        ...
        try:
            ...
            if selected_market == "A股":
                # 如果选择的是A股，则同时获取上证和深证的股票
                self.root.after(0, lambda: self._add_log("获取上证和深证股票列表...", "info"))
            ...
            # 更新日志显示
            if selected_market == "A股":
                self.root.after(0, lambda: self._add_log(f"获取到{len(stock_list)}只A股市场股票", "info"))
            else:
    ...
    def _get_stock_details(self):
        ...
        try:
            ...
            if selected_market == "A股":
                ...
                sh_stocks = self.data_fetcher.get_stock_list("SH")
                sz_stocks = self.data_fetcher.get_stock_list("SZ")
                self.detailed_info = self.data_fetcher.get_detailed_info(sh_stocks + sz_stocks)
            else:
                # 获取指定市场的股票列表
                stock_list = self.data_fetcher.get_stock_list(selected_market)
                self.detailed_info = self.data_fetcher.get_detailed_info(stock_list)
        except Exception as e:
            self._handle_error(f"获取股票详细信息失败: {str(e)}")
    ...
    def _update_ui_with_results(self):
        ...
        if hasattr(self, 'partial_match') and self.partial_match:
            ...
            if selected_market == "A股":
                ...
                initial_stock_count = sh_count + sz_count
                summary += f"初始股票数: {initial_stock_count}（上证{sh_count}+深证{sz_count}）\n"
            else:
                # 其他单一市场
                initial_stock_count = len(self.data_fetcher.get_stock_list(selected_market))
                summary += f"初始股票数: {initial_stock_count}\n"
