...
class StockDataFetcher:
    ...
    def get_stock_list(self, market="SH"):
        ...
        if "+" in market:
            ...
            combined_stocks = []
            for single_market in markets:
                if single_market in self.market_mapping:
                    market_stocks = self.get_stock_list(single_market)
                    combined_stocks.extend(market_stocks)
                    logger.info(f"获取{single_market}市场股票列表成功，共{len(market_stocks)}只股票")
                else:
                    logger.error(f"不支持的市场代码: {single_market}")
            
            logger.info(f"组合市场{market}获取股票列表成功，共{len(combined_stocks)}只股票")
    ...
    def filter_by_eight_steps(self, stock_codes, callback=None):
        """
        使用尾盘八大步骤进行筛选，并返回详细结果
        ...
        Returns:
        --------
        tuple (result_stocks, detailed_info)
            result_stocks: 符合所有条件的股票代码列表
            detailed_info: 详细信息，包含每支股票的筛选结果
        """
        logger.info(f"开始对{len(stock_codes)}只股票执行尾盘八大步骤筛选")
        
        # 执行八大步骤筛选
        filtered_stocks = self.apply_all_filters(stock_codes, step_callback=callback)
        
        # 保存筛选结果供后续分析
        self.last_filtered_stocks = filtered_stocks
        
        # 获取筛选后股票的详细信息
        detailed_info = []
        if filtered_stocks:
            detailed_info = self.get_detailed_info(filtered_stocks)
            logger.info(f"筛选完成，找到{len(filtered_stocks)}只符合尾盘八大步骤的股票")
        else:
            # 如果没有完全符合的股票，看是否有部分匹配的结果
            if hasattr(self, 'partial_results') and self.partial_results:
                # 获取部分匹配股票的详细信息
                detailed_info = self.get_detailed_info(self.partial_results)
                logger.info(f"未找到完全符合的股票，返回{len(self.partial_results)}只部分匹配的股票")
        
        return filtered_stocks, detailed_info
