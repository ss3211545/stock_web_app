...
class TailMarketStockApp:
    ...
    def _update_kline_chart(self, stock_code):
        ...
        try:
            ...
            # 绘制K线图
            dates = mdates.date2num(kdata['dates'])
            ohlc = list(zip(dates, kdata['opens'], kdata['highs'], kdata['lows'], kdata['closes']))
            candlestick_ohlc(ax1, ohlc, width=0.6, colorup='red', colordown='green')
            
            # 绘制均线
            if 'ma5' in kdata:
                ax1.plot(dates, kdata['ma5'], 'b-', label='MA5')
            if 'ma10' in kdata:
                ax1.plot(dates, kdata['ma10'], 'y-', label='MA10')
            if 'ma20' in kdata:
                ax1.plot(dates, kdata['ma20'], 'g-', label='MA20')
            if 'ma60' in kdata:
            ...
            # 绘制成交量图
            ax2.bar(dates, kdata['volumes'], color='blue', alpha=0.5)
