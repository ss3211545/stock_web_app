...
class StockDataFetcher:
    ...
    def get_best_data_source(self, data_type='realtime'):
        ...
        """
        # 从策略配置中获取基础数据源排序
        base_sources = self.data_source_strategy.get(data_type, ['sina', 'eastmoney', 'akshare', 'tencent', 'ifeng'])
        ...
        for source in base_sources:
            ...
            if data_type == 'realtime':
                # 实时数据更看重速度
                score = success_rate * 0.5 + time_factor * 0.1 + speed_score * 0.4
            elif data_type == 'kline':
                # K线数据更看重成功率
                score = success_rate * 0.7 + time_factor * 0.2 + speed_score * 0.1
            else:
                # 股票列表兼顾成功率和速度
