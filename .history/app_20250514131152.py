...
class TailMarketStockApp:
    ...
    def _init_ui(self):
        ...
        # 创建八大步骤详解内容标签页的内容
        self.step_detail_text = tk.Text(filter_detail_frame, wrap=tk.WORD)
    ...
    def _update_kline_chart(self, stock_code):
        ...
        try:
            ...
            # 获取K线数据
            kdata = self.data_fetcher.get_kline_data(stock_code)
            if not kdata:
                return
            
            # 创建K线图和成交量图
            ax1 = self.fig.add_subplot(2, 1, 1)  # K线图
            ax2 = self.fig.add_subplot(2, 1, 2, sharex=ax1)  # 成交量图
            ...
            # 绘制收盘价线
            ax1.plot(dates, closes, 'b-', label='收盘价')
            
            # 绘制均线
            if 'ma5' in kdata:
                ax1.plot(dates, kdata['ma5'], 'g-', label='MA5')
            if 'ma10' in kdata:
                ax1.plot(dates, kdata['ma10'], 'y-', label='MA10')
            if 'ma20' in kdata:
                ax1.plot(dates, kdata['ma20'], 'm-', label='MA20')
            if 'ma60' in kdata:
                ax1.plot(dates, kdata['ma60'], 'r-', label='MA60')
            ...
            # 设置y轴范围
            if kdata['highs'] and kdata['lows']:
                min_price = min(kdata['lows'])
                max_price = max(kdata['highs'])
                margin = (max_price - min_price) * 0.1
                ax1.set_ylim(min_price - margin, max_price + margin)
            
            # 绘制成交量图
            ax2.bar(dates, kdata['volumes'], color='blue', alpha=0.5, label='成交量')
            ...
            ax1.set_title(f"{stock_name} ({stock_code}) 价格走势图")
            ax1.set_ylabel('价格')
            ax2.set_ylabel('成交量')
            ax2.set_xlabel('日期')
    ...
    def _update_detail_info(self, stock_info):
        ...
        if 'market_cap' in stock_info:
            detail_text += f"市值: {stock_info['market_cap']:.2f}亿\n"
        if 'volume_ratio' in stock_info:
            detail_text += f"量比: {stock_info['volume_ratio']:.2f}\n"
        if 'ma5' in stock_info:
            detail_text += f"5日均线: {stock_info['ma5']:.2f}\n"
        if 'ma10' in stock_info:
            detail_text += f"10日均线: {stock_info['ma10']:.2f}\n"
        if 'ma20' in stock_info:
            detail_text += f"20日均线: {stock_info['ma20']:.2f}\n"
        if 'ma60' in stock_info:
            detail_text += f"60日均线: {stock_info['ma60']:.2f}\n"
    ...
    def _update_steps_analysis(self, stock_code):
        ...
        if not found or not stock_info:
            return
        
        # 准备显示的步骤分析信息
        steps_text = f"{stock_info['name']} ({stock_code}) 八大步骤筛选分析\n\n"
        
        # 获取股票对应的步骤数据
        if hasattr(self.data_fetcher, 'get_steps_data'):
            try:
                step_data = self.data_fetcher.get_steps_data(stock_code)
                ...
                if step_data:
                    ...
                    for i, step in enumerate(self.step_descriptions):
                        ...
                        if i in step_data:
                            ...
                            steps_text += f"步骤 {i+1}: {step_name} - {status}\n"
                            steps_text += f"  要求: {required}\n"
                            steps_text += f"  实际: {value}\n"
                            
                            if details:
                                steps_text += f"  详情: {details}\n"
                        else:
                            steps_text += f"步骤 {i+1}: {step_name} - 数据不可用\n"
                            
                        steps_text += "\n"
                else:
                    steps_text += "未找到该股票的步骤分析数据。\n"
            except Exception as e:
                steps_text += f"获取步骤数据时出错: {str(e)}\n"
        else:
            # 如果没有步骤数据，显示基本信息和筛选建议
            steps_text += "股票基本信息:\n"
            steps_text += f"  价格: {stock_info['price']:.2f}\n"
            steps_text += f"  涨跌幅: {stock_info['change_pct']:.2f}%\n"
            
            if 'turnover_rate' in stock_info:
                steps_text += f"  换手率: {stock_info['turnover_rate']:.2f}%\n"
            if 'volume_ratio' in stock_info:
                steps_text += f"  量比: {stock_info['volume_ratio']:.2f}\n"
            if 'market_cap' in stock_info:
                steps_text += f"  市值: {stock_info['market_cap']:.2f}亿\n"
            ...
            if self.partial_match:
                if self.max_step > 0:
                    steps_text += f"该股票符合前{self.max_step}步筛选条件，但未能通过全部八大步骤。\n"
                    steps_text += "建议关注，但需要进一步分析其他指标后再做决策。\n"
                else:
                    steps_text += "该股票未能通过八大步骤筛选，仅作为当日涨幅较高的股票展示。\n"
                    steps_text += "不建议作为投资标的，除非有其他支持因素。\n"
            else:
                steps_text += "该股票成功通过八大步骤筛选，符合尾盘选股策略的全部要求。\n"
                steps_text += "建议优先考虑关注，具备较好的短期上涨潜力。\n"
